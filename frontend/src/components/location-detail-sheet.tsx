...
import { useRealTimeConditions, useLocationTimeline } from '@/lib/hooks/useApiData';
...
interface LocationDetailSheetProps {
    ...
    isOpen: boolean;
    onClose: () => void;
}export function LocationDetailSheet({ location, isOpen, onClose }: LocationDetailSheetProps) {
    const {
        data: realTimeData,
                ...
                <div className="space-y-6 mt-6 px-4">
                            ...
                            <CardContent className="p-6">
                                <div className="text-center text-red-600">
                                    ...
                                    <p className="text-sm text-gray-600 mt-1">
                                        {realTimeError?.message || timelineError?.message || 'Unknown error'}
                                    </p>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <CardContent>
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <Badge className={getRiskLevelColor(location.flood_risk.toLowerCase())}>
                                        {location.flood_risk}
                                    </Badge>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p className="font-medium text-blue-600">{location.river_level.toFixed(2)}m</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-600">Change Rate</p>
                                    <p className={`font-medium ${location.change_rate > 0 ? 'text-red-600' : location.change_rate < 0 ? 'text-green-600' : 'text-gray-600'}`}>
                    ...
                    {realTimeData && (
                        <>
                            ...
                            <Card>
                                <CardHeader>
                                    <CardTitle className="text-lg flex items-center gap-2">
                                        🌊 River Conditions
                                        <Badge className={getRiskLevelColor(realTimeData.river_conditions.flood_risk)}>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="font-medium">
                                                ...
                                                {realTimeData.river_conditions.change_rate.toFixed(3)}m/15min
                                            </p>
                                        </div>
                                        <div>
                                            <p className="text-sm text-gray-600">Sensor</p>
                                            <p className="font-medium">{realTimeData.river_conditions.sensor_id}</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                            ...
                            <Card>
                                <CardHeader>
                                    <CardTitle className="text-lg">🌤️ Weather Conditions</CardTitle>
                                </CardHeader>
                                <CardContent>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                        ...
                                        <div>
                                            <p className="text-sm text-gray-600">Rainfall Rate (hourly)</p>
                                        ...
                                        <div>
                                            ...
                                            <p className="font-medium">{realTimeData.weather.temperature_c.toFixed(1)}°C</p>
                                        </div>
                                        <div>
                                            <p className="text-sm text-gray-600">Humidity</p>
                                            <p className="font-medium">{realTimeData.weather.humidity_percent}%</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent>
                                    <div className="grid grid-cols-2 gap-4 mb-4">
                                        <div>
                                            ...
                                            <p className="text-xl font-bold">{realTimeData.social_activity.post_count}</p>
                                        </div>
                                        <div>
                                            <p className="text-sm text-gray-600">Sentiment</p>
                                            <p className="font-medium flex items-center gap-1">
                                                {getSentimentIcon(realTimeData.social_activity.sentiment_level)}
                                    ...
                                    {realTimeData.social_activity.recent_posts.length > 0 && (
                                        <div>
                                            ...
                                            <div className="space-y-2">
                                                {realTimeData.social_activity.recent_posts.slice(0, 3).map((post, index) => (
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <div className="space-y-3">
                                        ...
                                        <div>
                                            <p className="text-sm text-gray-600 font-medium">Correlation Analysis</p>
                                            <p className="text-sm">{realTimeData.insights.correlation}</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                    ...
                    {timelineData && (
                        <Card>
                            <CardHeader>
                                <CardTitle className="text-lg">📊 Recent Timeline (5 minutes)</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <div className="mb-4">
                                    <div className="grid grid-cols-3 gap-4 text-sm">
                                        <div>
                                            ...
                                            <p className="font-medium">{timelineData.summary.trend}</p>
                                        </div>
                                        <div>
                                            <p className="text-gray-600">Max Level</p>
                                            <p className="font-medium">{timelineData.summary.max_level.toFixed(2)}m</p>
                                        </div>
                                        <div>
                                            <p className="text-gray-600">Min Level</p>
                                            <p className="font-medium">{timelineData.summary.min_level.toFixed(2)}m</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2 max-h-32 overflow-y-auto">
                                    {timelineData.timeline.slice(0, 5).map((point, index) => (
                                        <div key={index} className="flex justify-between items-center text-sm p-2 bg-gray-50 rounded">
                                            <span>{dataTransformers.formatTimestamp(point.timestamp)}</span>
                                            <span className="font-medium">{point.river_level.toFixed(2)}m</span>
                                            <Badge size="sm" className={getRiskLevelColor(point.flood_risk)}>
                                                {point.flood_risk}
                                            </Badge>
                                        </div>
                                    ))}
                    ...
                    {realTimeData && (
                        <div className="text-center text-sm text-gray-500">
                            Last updated: {dataTransformers.formatRelativeTime(realTimeData.timestamp)}
                        </div>
                    )}
                </div>
            </SheetContent>
