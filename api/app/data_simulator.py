            ...
            with SessionLocal() as db:
                ...
                for sensor_id, (location_name, lat, lon) in SENSOR_LOCATIONS.items():
                    ...
                    if cycle < 30:  # First 1.5 minutes: heavy rain scenario
                        ...
                        pred_wind = float(round(np.random.uniform(20, 30), 1))  # Strong winds
                        pred_temp = float(round(np.random.uniform(24, 26), 1))  # Cooler temperatures during storm
                        pred_humid = float(round(np.random.uniform(85, 95), 1))  # High humidity during storm
                        ...
                        pred_wind = float(round(np.random.uniform(15, 25), 1))  # Gusty winds
                        pred_temp = float(round(np.random.uniform(25, 27), 1))  # Slightly warmer
                        pred_humid = float(round(np.random.uniform(80, 90), 1))  # Still high humidity
                        ...
                        pred_wind = float(round(np.random.uniform(10, 15), 1))  # Moderate winds
                        pred_temp = float(round(np.random.uniform(26, 28), 1))  # Warming up
                        pred_humid = float(round(np.random.uniform(70, 85), 1))  # Decreasing humidity
