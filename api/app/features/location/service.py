...
def get_social_summary(session: Session) -> Dict[str, Any]:
    """Get aggregate social media sentiment across all locations"""
    locations = get_all_locations(session)
    ...
    for location in locations:
        social = get_latest_social_data(session, location.name)
        if social:
            post_count = int(social.post_count) if social.post_count else 0
            score = float(social.sentiment_score) if social.sentiment_score else 0.0
    ...
    avg_sentiment = round(sentiment_sum / sentiment_count, 3) if sentiment_count else 0.0
    ...
    if avg_sentiment < -0.3:
        ...
        classification = "highly_positive"

    return {
        "overall_avg_sentiment": avg_sentiment,
        "sentiment_classification": classification,
        "total_posts": total_posts,
        "locations_count": sentiment_count,
        "locations": per_location,
        "timestamp": datetime.now(timezone.utc).isoformat(),
    }


def get_risk_summary(session: Session) -> Dict[str, Any]:
    """Get all locations grouped by flood risk level"""
    locations = get_all_locations(session)
    ...
    for location in locations:
        river = get_latest_river_data(session, location.name)
        if river:
            level_m = float(river.river_level_m)
            change = float(river.change_in_level_m)
            risk = calculate_flood_risk(level_m)
            grouped[risk].append({
                "name": location.name,
                "risk_level": risk,
                "river_level_m": round(level_m, 3),
                "change_rate_m": round(change, 3),
                "sensor_id": location.sensor_id,
            })

    total = sum(len(v) for v in grouped.values())
    high_risk_count = len(grouped["CRITICAL"]) + len(grouped["HIGH"])
