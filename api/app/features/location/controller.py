...
async def get_social_summary(session: SessionDep):
    """Get aggregate social media sentiment across all locations"""
    try:
        summary = service.get_social_summary(session)
        return summary
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error getting social summary: {str(e)}")


@router.get("/risk/summary")
async def get_risk_summary(session: SessionDep):
    """Get all locations grouped by flood risk level"""
    try:
        summary = service.get_risk_summary(session)
        return summary
    except Exception as e:
