...
class PlaybackController:
    ...
    def play(self, speed: float = None):
        ...
        if self.state == PlaybackState.PAUSED and self.paused_time:
            ...
            self.virtual_start_time = datetime.now() - timedelta(
                seconds=self.virtual_index * 15 / self.speed
            )
        else:
            # Start fresh
            self.virtual_start_time = datetime.now()
        ...
        if self.state == PlaybackState.STOPPED:
            ...
            if (
                ...
                if new_index >= self.loop_end:
                    new_index = self.loop_start + (
                        (new_index - self.loop_start) % loop_length
                    )
            else:
                # Stop at end of data
    ...
    def get_status(self, max_data_points: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "virtual_time": (
                self.get_virtual_timestamp().isoformat()
                        ...
                        events.append(
                            {
                                ...
                                "severity": (
                                    "low"
                                    if level < 3.0
                                    else "medium" if level < 4.0 else "high"
                                ),
...
def load_fallback_data():
    ...
    print(
        f"Generated sample data: {len(river_data)} river, {len(weather_data)} weather, {len(social_data)} social points"
    )
...
async def api_forecast_river(
    ...
    horizon_seconds: int = Query(1800, ge=15, le=7200),
    ...
    summary = {
        ...
        "flood_events": len(
            [
                ...
                if e["type"] in ["flood_threshold_crossed", "critical_level"]
            ]
        ),
        "weather_events": len(
            [e for e in detected_events if e["type"] == "heavy_rainfall"]
        ),
    }
...
async def add_bookmark(request: BookmarkRequest):
    ...
    index = (
        request.index
        if request.index is not None
        else playback_controller.virtual_index
    )
    playback_controller.add_bookmark(request.name, index)
    return {"success": True, "bookmark": request.name, "index": index}
...
async def get_data_range(
    start_index: int, end_index: int, location: Optional[str] = None
):
    ...
    try:
        ...
        if location:
            ...
            locations = list(
                set(d.get("location") for d in river_data if d.get("location"))
            )
                    ...
                    result.append(
                        {
                            ...
                            "flood_risk": calculate_flood_risk(
                                river_point.get("river_level_m", 0)
                            ),
                            "rainfall": (
                                weather_point.get("actual_rainfall_mm", 0)
                                if weather_point
                                else 0
                            ),
                        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting data range: {str(e)}"
        )
...
async def get_system_update():
    ...
    try:
        ...
        available_locations = list(
            set([d["location"] for d in river_data if "location" in d])
        )
        ...
        for location_data in locations_data:
            if location_data["flood_risk"] in [
                "ELEVATED",
                "MEDIUM",
                "HIGH",
                "CRITICAL",
        ...
        return {
            ...
            "system_status": {
                "active_sensors": len(
                    set([d.get("sensor_id") for d in river_data if d.get("sensor_id")])
                ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting system update: {str(e)}"
        )
...
async def get_real_time_conditions(location: str):
    ...
    try:
        ...
        if not river_point:
            river_point = {
                "river_level_m": 2.0,
                "change_in_level_m": 0.01,
                "sensor_id": "CR-999",
            }

        if not weather_point:
            weather_point = {
                "actual_rainfall_mm": 1.0,
                "actual_temperature_c": 27.0,
                "actual_humidity_percent": 75,
            }
        ...
        return {
            ...
            "river_conditions": {
                ...
                "trend": (
                    "rising"
                    if river_point["change_in_level_m"] > 0
                    else "falling" if river_point["change_in_level_m"] < 0 else "stable"
                ),
                "flood_risk": flood_risk,
                "sensor_id": river_point.get("sensor_id", "N/A"),
            },
            ...
            "social_activity": {
                ...
                "sentiment_level": (
                    "Highly Negative"
                    if social_point["sentiment_score"] < -0.5
                    else (
                        "Negative"
                        if social_point["sentiment_score"] < -0.3
                        else "Neutral"
                    )
                ),
                "activity_level": (
                    "HIGH"
                    if social_point["post_count"] >= 6
                    else "MEDIUM" if social_point["post_count"] >= 3 else "LOW"
                ),
            },
            "playback_info": playback_controller.get_status(len(river_data)),
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting real-time data: {str(e)}"
        )
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        app, host="0.0.0.0", port=8001, reload=True
